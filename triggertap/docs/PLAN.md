...
### 명세 변경 (사용자 요청, 2026-09-24)

명세 1장의 "무작위 타이밍·무작위 좌표 이동은 넣지 않는다"를 사용자 요청으로 바꿨다. 탭마다 시작 간격을 ±N ms(기본 15, 최대 30), 좌표를 반경 R px(기본 10, 최대 40) 원 안에서 고르게 흔든다. 0이면 끈다.
- 간격은 80~200ms 범위와 "누름 + 10ms 이상" 안으로 제한한다. 좌표가 화면 밖이나 조작 패널 위로 나가면 원래 좌표를 쓴다.
- 흔들림은 뷰 보안 필터(`accessibilityDataSensitive` 등)와 무관하다. 이 필터는 입력의 출처를 보고 거르므로 타이밍·위치를 바꿔도 결과가 같다.
- 반경은 대상 버튼보다 충분히 작게 둔다. 버튼 가장자리 가까이에 표식을 두면 일부 탭이 버튼 밖으로 나갈 수 있다.
