...
> 상태: **v0.3 — 흐름 모델(15절)과 구단 전술 기본 on.** 웹 엔진(`web/engine/match.js`)에 먼저 들어갔고 C# 은 아직이다(parity 는 기본 설정=흐름 off 로 비트 일치 유지). v0.2 = 프로토타입 구현 완료(`sim/`), 몬테카를로 캘리브레이션 4차(v0.1) + **포지션 가치 재캘리브레이션 5회(v0.2)** 반영. 이 문서의 상수는 `SimConfig` 기본값과 일치한다. v0.2 에서 바뀐 판정식은 6.4·6.6~6.9 절에 표시했고, 결과는 밸런스 리포트의 v0.2 절에 있다.
...
- [ ] **AI 교체**: 피로·부진 기반 자동 교체 정책 — 12명 로스터(선수 D)와 함께. 타임아웃은 v0.3 흐름 모델로 들어갔다(15절).
...
남은 것: 플레이어가 직접 작전타임을 부르고 세트 간 작전을 고르는 것(게임성 2단계), 부진·피로 기반 교체(12명 로스터), C# 포팅.
