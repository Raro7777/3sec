...
| 보정 | 식 | 성격 |
|---|---|---|
| 리베로 역할 게인 | 리베로가 리시브/디그/커버할 때 `r_eff = 80 + 2.0 × (r − 80)` | **의도적 보정.** 배구적 근거보다 카드 상품성이 이유다. 대칭이라 나쁜 리베로는 더 나쁘다(R 리베로 60 → 실효 40). 기준 80 = 프로 주전급(생성기 프로파일 81) |
...
**튜닝 손잡이.** 비율을 낮추려면 `Receive/Dig.LiberoRoleGain`(2.0)·`Block.MbStrengthWeight`(1.8)·`Attack.MbDecoyK`(100, 클수록 약함) 순으로 만진다. 게인 1.0·가중 1.0·`MbDecoyK` 0·디그 가중 1.5·블렌드 0.3·속공 가용 0.6 으로 되돌리면 v0.1 과 비트 단위로 같은 결과가 나온다(회귀 확인 완료).
