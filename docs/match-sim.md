# 경기 시뮬레이션 상세 설계 (match-sim v0.2)

> 상태: **v0.4 — 경기 엔진 2단계(16~19절: 선수 교체·컨디션·경상·케미·AI 구단 운영) + v0.3 흐름 모델(15절)과 구단 전술 기본 on.** 웹 엔진(`web/engine/match.js`)에 먼저 들어갔고 C# 은 아직이다(parity 는 기본 설정=흐름 off 로 비트 일치 유지). v0.2 = 프로토타입 구현 완료(`sim/`), 몬테카를로 캘리브레이션 4차(v0.1) + **포지션 가치 재캘리브레이션 5회(v0.2)** 반영. 이 문서의 상수는 `SimConfig` 기본값과 일치한다. v0.2 에서 바뀐 판정식은 6.4·6.6~6.9 절에 표시했고, 결과는 밸런스 리포트의 v0.2 절에 있다.
...
- 1세트 첫 서브: `HomeServesFirst = true`(기본, 밸런스 측정용) 또는 시드 동전. 2~4세트는 교대, 5세트는 시드 동전. **최종 세트 규칙이 우선**이므로 `SetsToWin = 1`(육성 평가전처럼 단판 1세트) 설정에서는 1세트가 곧 최종 세트가 되어 첫 서브가 항상 동전이고 목표점도 `PointsToWinFinalSet` 을 쓴다.
...
| 리베로 역할 게인 (v0.2) | 리시버가 리베로면 실효 리시브 레이팅을 `80 + 2.0 × (ReceiveRating − 80)` 으로 스트레치(실효 배수 적용 후). 기준 80 에서 효과 0, 그 위·아래 편차가 2배 — 6.9 절 |
...
|---|---|---|---|---|---|
| Quick 속공 | 전위 MB | 1 / **0.8** / 0 (v0.2, 이전 0.6) | +0.30 | ×1.0 | −0.15 |
...
**속공 위협(디코이, v0.2)**: 후보 목록에 속공이 실제로 들어 있었는데(패스 A/B·전위 MB 존재·`QuickWeight > 0`) 결과적으로 오픈/시간차/후위 공격이 선택되면, 그 속공 후보 MB(공격 레이팅 최고)가 상대 블로커를 붙잡는다:
`decoy = clamp((MbAttackRating_eff − 70) / 100, ±0.35) × min(1, 전술 속공 비중 / 0.15)`. 블로킹 로짓에서 `decoy × 1.0` 을 빼고 킬 로짓에 `decoy × 0.5` 를 더한다. 속공이 선택됐거나 덤프이거나 속공이 후보에 없었으면 0. 전술 속공 비중이 15% 미만이면 비례 축소되어 "속공을 안 쓰면서 위협만 누리는" 편법이 없다. 상수: `Attack.MbDecoyK`(100)·`MbDecoyMaxLogit`(0.35)·`MbDecoyBlockShare`(1.0)·`MbDecoyKillShare`(0.5)·`MbDecoyFullQuickShare`(0.15).
...
| 블로킹 강도 | `wmean(BlockRating_eff) + 8 × (블로커 수 − 2)` — v0.2: 가중 평균, **MB 블로커 가중 1.8**·그 외 1.0 (`Block.MbStrengthWeight`). 2인 블록에서 MB 가 64%. "블록을 닫는 것은 미들블로커" |
| 블로킹 득점 | `Contest(0.115, 강도 − AttackRating, 120, 유형+세트품질+예측 로짓 − 디코이)` (디코이는 6.6) |
...
| 디거 선택 | 수비팀 비블로커. 가중치 리베로 **2.4**(v0.2, 이전 1.5) · OH 1.0 · OP 0.8 · MB 0.6 · S 0.5, 전위 비블로커 ×0.5 → 리베로 디그 비중 약 40% |
| 리베로 역할 게인 (v0.2) | 디거·커버어가 리베로면 실효 디그 레이팅을 `80 + 2.0 × (DigRating − 80)` 으로 스트레치(6.9 절). 팀 평균 항에는 적용하지 않는다 |
| 수비 레이팅 | `dig = 0.8·DigRating(디거) + 0.2·평균 DigRating(비블로커)` (v0.2, 이전 0.7/0.3); `defense = 0.65·dig + 0.35·wmean BlockRating(블로커, MB 가중 1.8)` (블로커 없으면 dig) |
| 킬 | `Contest(0.41, AttackRating − defense, 120, 유형 + 세트품질 − 예측 − 느린공 + 0.5×디코이 + 퍼스트볼 0.35 또는 트랜지션 0.05×n + 홈이점)` |
...
### 6.9 포지션 가치 보정 (v0.2) — 가챠 공정성을 위한 의도적 보정

**문제.** v0.1 에서 "해당 선수 1명만 전 스탯 +15" 의 승률 효과는 OH +14.6 / OP +12.8 / S +12.0 / MB +8.3 / L +7.8 %p 였다. 배구답게는 맞지만(리베로는 리시브·디그 두 판정에만, MB 는 코트에 절반만 있다), 이 게임은 카드 수집형 가챠라 **SSR 리베로·미들블로커를 뽑은 유저가 손해를 느끼면 안 된다.** 기획 결정: 시뮬 가중치로 부분 보정 — MB·L 1인 가치를 OH 1인의 80~85% 로. 포지션 역할 차이(OH 가 최고)는 유지하고 격차만 줄인다.

**왜 "비중" 이 아니라 "영향력" 인가.** 리베로의 리시브 비중은 이미 43%(디그 31%)였다. 낮은 가치의 원인은 (1) 참여 판정이 두 종류뿐이고, (2) 디그는 선택 디거 0.7 × (블로커 있을 때) 0.65 = 0.455 만 반영돼 희석되며, (3) 모든 대결 k 가 120 으로 평탄해 개인 +15 가 로짓 0.125 에 그친다는 데 있다. k 를 낮추면 OH 도 같이 오르므로 비율은 안 움직인다. 따라서 **리베로·MB 전용 경로**가 필요했다.

**보정 4종(모두 기준 레이팅에서 효과 0 → 동급 팀 베이스라인 불변):**

| 보정 | 식 | 성격 |
|---|---|---|
| 리베로 역할 게인 | 리베로가 리시브/디그/커버할 때 `r_eff = 80 + 2.0 × (r − 80)` | **의도적 보정.** 배구적 근거보다 카드 상품성이 이유다. 대칭이라 나쁜 리베로는 더 나쁘다(R 리베로 60 → 실효 40). 기준 80 = 프로 주전급(생성기 프로파일 81). **클램프 없음** — 5.1 절의 "0~100 스케일"에서 벗어난 실효 레이팅이 나올 수 있다(스탯 0 리베로 → 음수, 만렙 리베로 → 100 초과). 판정은 로짓 차이만 쓰고 `SimMath.Logit/Sigmoid` 가 확률을 유한하게 묶으므로 발산하지 않지만, 스탯 하위·상위 끝에서 리베로 가치가 선형보다 빠르게 벌어진다는 뜻이다(회귀 테스트 `LiberoRoleGain_ExtremeRatings_KeepProbabilitiesValid`) |
| 리베로 디그 비중 | `Dig.WeightLibero` 1.5→2.4, `DigTeamBlend` 0.3→0.2 | 실제 배구 수준(리베로 디그 ≈ 40%)으로 맞춘 것 |
| MB 블로킹 가중 | 블로킹 강도·킬 대결의 블로킹 몫이 MB 1.8 : 그 외 1.0 의 가중 평균 | 실제 배구(미들이 블록을 닫는다)와 부합하는 역할 가중 |
| 속공 위협(디코이) | 속공이 살아 있는 공격에서 전위 MB 의 공격 레이팅이 윙 공격의 피블로킹↓·킬↑ (6.6) | 실제 배구 메커니즘("좋은 미들이 윙을 살린다"). 전술 속공 비중에 비례해 몰빵/오픈-only 악용 차단 |

여기에 `QuickAvailGoodPass` 0.6→0.8 로 속공 비중을 20→22.5% 로 올렸다.

**결과(리포트 v0.2-2).** 14,000경기 풀링: OH +12.6 / L +11.3(90%) / MB +11.1(88%) / S +11.0 / OP +10.9. 목표 상한을 3~5%p 상회(SE ±4.5%p). 부작용으로 OH·S·OP 절대값이 1~2%p 내려갔다(팀 강도 분산이 L·MB 로 재분배되며 로지스틱 기울기가 희석). 팀 전체 민감도는 +5: 72→76%, +10: 89→91% 로 소폭 상승, 7개 KPI 는 전부 유지, 몰빵 전술은 여전히 손해.

**튜닝 손잡이.** 비율을 낮추려면 `Receive/Dig.LiberoRoleGain`(2.0)·`Block.MbStrengthWeight`(1.8)·`Attack.MbDecoyK`(100, 클수록 약함) 순으로 만진다. 게인 1.0·가중 1.0·`MbDecoyK` 0·디그 가중 1.5·블렌드 0.3·속공 가용 0.6 으로 되돌리면 v0.1 과 비트 단위로 같은 결과가 나온다 — 2026-09 리뷰에서 v0.1 커밋(`f6ad79f`)의 Core 를 그대로 빌드해 6개 시나리오 160경기(기본·속공 100%·리베로 중심 + 강서브·극단 리베로 99/0·극단 MB 99)의 `Signature()` 가 전부 일치함을 확인했고, 그중 6건을 골든 값으로 `PositionValueRegressionTests` 에 고정해 두었다.
...
| 파라미터 | 범위 | 효과 |
|---|---|---|
| `QuickWeight / OpenWeight / BackRowWeight / DelayedWeight` | ≥0 | 6.6 의 후보 가중치. 비중이 0.45 를 넘는 유형은 예측 페널티. 속공 비중이 0.15 미만이면 디코이 효과 비례 축소(v0.2) |
...
| 흐름 (v0.3, 웹) | `config.flow` (기본 `enabled:false`) | 연속 득점 압박·자신감·작전타임·세트 간 서브 조정 — 15절 |
...
1. **개별 대결 기울기 vs 팀 승률 민감도의 긴장.** 배구는 한 랠리에 6~8개의 대결이 있고 한 세트에 ~45 랠리가 있어, 대결당 작은 우위가 세트/경기 승률로 크게 증폭된다. 초기값 k≈22 에서는 팀 전원 +5 가 승률 97% 를 만들었다. 리그가 "정해진 사다리"가 되지 않도록 k=120(세트 40, 서브/리시브 60~70)으로 평탄화하여 v0.1 에서 **전원 +5 ≈ 72%, +10 ≈ 89%, +20 ≈ 99%** 를 기본으로 삼았다(v0.2 는 포지션 보정으로 스탯 채널이 늘어 **+5 ≈ 76%, +10 ≈ 91%, +20 ≈ 99.6%**). 이 수치는 `StatSensitivity` 하나로 통째로 조절할 수 있다.
2. **몰빵 전술 페널티.** 순수 확률만으로는 속공 100% 가 최적이 되어 슬라이더가 무의미해진다. 예측 가능성 페널티(6.6)로 "섞어야 이득"인 구조를 만들었다.
3. **포지션 가치 보정(v0.2).** 리베로·MB 의 1인 가치를 OH 의 80~85% 로 끌어올리되 완전 동등은 피한다(6.9). 보정은 전부 기준 레이팅에서 0 이라 팀 KPI 를 흔들지 않고, 게인 상수를 1.0 으로 되돌리면 v0.1 로 정확히 복귀한다.
...
| 포지션 보정 회귀 | **중립값 = v0.1 골든 서명 6건**(기본·속공 전용·극단 리베로/MB) / 기본값이 중립값과 다름 / 리베로 스트레치 극단값에서 확률 NaN·범위 이탈 없음 / 좋은 리베로 &gt; 나쁜 리베로 |
| 민감도 | 전원 +10 → 승률 > 65% & 기준 대비 +15%p 이상 / −10 < 0 < +5 < +10 ≤ +20, +20 > 90% / 포지션별 +20 모두 양(+3%p 이상) / **포지션 가치 비율(v0.2): MB 1명·L 1명 +15 의 효과가 OH 1명의 75~95%**(시나리오당 5,000경기, 약 10초) / 강서브: 에이스·범실 동시 상승 / 전술 배분 반영 / 클러치 mental 95 vs 25 → 클러치 랠리 승률 차이 |
...
- [x] **AI 교체**: 피로·부진 기반 자동 교체 정책 — 12명 로스터와 함께 v0.4 로 들어갔다(16절). 타임아웃은 v0.3 흐름 모델(15절).
- [ ] **C# 포팅**: 15절 흐름 모델과 `Timeout` 이벤트를 `sim/` 에 옮기고 parity 를 흐름 on 으로도 잰다.
- [ ] **후위 OH 파이프**: 현재 후위 공격은 OP 우선(없을 때만 OH). OH 파이프 옵션과 전술 가중치 추가 검토.
- [ ] **서브 표적 전술**: 현재는 공격성에 비례한 약한 리시버 표적. 명시적 "누구를 노릴지" 슬라이더 검토.
- [x] **포지션 가치 균형(v0.2 완료)**: 14,000경기 풀링 기준 OH +12.6 / MB +11.1(88%) / L +11.3(90%). 남은 것 ↓
- [ ] **포지션 비율 트림**: 목표 상한(85%)을 3~5%p 상회(SE ±4.5%p). 정중앙을 원하면 `LiberoRoleGain` 2.0→1.8, `MbStrengthWeight` 1.8→1.6 후 시드 3개 × 4,000경기로 재확인(예상 82~84%).
- [ ] **리베로 역할 게인 기준(80) 재검토**: 실제 로스터(R 다수, 평균 55~65)에서는 R 리베로가 v0.1 보다 뚜렷이 약해진다(가온 vs 해솔 63.2→51.5%). 로스터 분포 확정 후 기준을 72~76 으로 낮출지 결정(낮추면 `AceBase`·`Receive.PerfectBase` 재조정 동반).
- [ ] **OH·S·OP 절대값 하락(각 −1~2%p)**: 분배 효과. 스타 OH 체감을 v0.1 수준으로 되돌리려면 `StatSensitivity` 1.1 이 필요하나 전원 +5 가 80% 로 올라간다. 기획 판단.
- [ ] **실제 로스터의 속공 100%**: 리시브가 약한 팀(가온)에서는 속공 몰빵이 손해가 아니다(51.5 vs 52.0%). `QuickAvailGoodPass` 0.8→0.7 또는 속공 전용 예측 페널티 검토.
- [ ] **전위 스위치**: 실제 배구는 서브 후 MB 가 3번(중앙)으로 이동하지만 시뮬은 로테이션 위치 그대로 블로킹한다. 구현하면 MB 블로킹 역할이 자연스러워져 `MbStrengthWeight` 를 낮출 수 있다.
...
## 15. 흐름 모델 (v0.3, 웹 엔진) — 경기에 흐름이 생긴다

"매 랠리가 독립 시행처럼 보인다"(테스터 피드백 "아직 테스트 버전 같다")에 대한 첫 답. 규칙만 바꾸고 그림은 필요 없다.
`web/engine/config.js` 의 `flow` 블록이고 **기본은 꺼져 있다** — parity 와 기본 `simulateMatch` 는 기존 판정과 비트 단위로 같다.
게임 층(`game.js` `gameSimConfig`, `season.js`)이 `FLOW.enabled` 로 켠다. RNG 를 추가로 소비하지 않는다(로짓만 더한다).

| 규칙 | 상수 | 내용 |
|---|---|---|
| 압박 | `streakStart 3` · `pressurePerPoint 0.06` · `pressureMax 0.20` | 상대가 3연속 득점 중이면 이 팀의 **서브 범실·공격 범실 로짓 +x, 리시브 품질 로짓 −x**. x = (연속−2)×0.06, 최대 0.20 |
| 멘탈 | `mentalRelief 0.5` | 압박 x 에 (1 − 0.5·(멘탈−50)/50) 을 곱한다 — 멘탈 100 은 절반, 멘탈 0 은 1.5배 (0.4~1.6 클램프) |
| 자신감 | `confidenceShare 0.5` | 연속 득점 중인 쪽의 **킬 로짓 +x/2** |
| 작전타임 | `timeoutsPerSet 2` · `timeoutAtStreak 3` | 당하는 쪽 감독이 상대 3연속에 남은 작전타임을 쓴다. 연속이 0 으로 초기화되어 압박·자신감이 사라진다. `EV.Timeout`(18, value = 끊은 연속 수) 이벤트 |
| 세트 간 조정 | `adjustTrailing +0.10` · `adjustLeading −0.05` | 세트를 뒤진 팀은 다음 세트 서브 공격성 +0.10(에이스도 범실도 ↑), 앞선 팀 −0.05. 동률이면 원래대로. 팀 전술 사본(`TeamMatchState.tactics`)에만 적용 |
| 결과 | `result.flow = { timeouts:[h,a], maxStreak:[h,a] }` | 표현·검증용 |

같이 켠 것: **구단 전술 기본 on** (`TACTICS.clubTactics`, `CLUB_TACTICS` 6구단 표 — 7절). 전에는 세이브 플래그 `useClubTactics` 가 기본 false 라 여섯 구단이 같은 전술로 뛰었다.

**측정 (`node web/season-check.mjs`, n=200 · seed 1 · 시즌 1~3, rev cc26a45 엔진).** `--no-club-tactics --no-flow` 는 v0.5.2 기준선을 비트 단위로 재현한다.

| 조합 | S1 승률 | S1 순위 | S1 우승 | 첫 우승 S2~3 | S3 골드 | 판정 |
|---|---|---|---|---|---|---|
| 둘 다 off (기준선) | 43.6% | 4.71 | 15.5% | 54.5% | 9350 | 9/9 |
| 구단 전술만 | 43.8% | 4.64 | 17.5% | 48.5% | 8435 | 7/9 |
| 흐름만 | 44.6% | 4.63 | 17.5% | 55.5% | 9022 | 9/9 |
| **둘 다 on — 채택** | **43.5%** | **4.92** | **12.0%** | **58.5%** | **8923** | **9/9** |

- 둘을 같이 켜면 원래 기준선(v0.5.1 이전, 시즌 1 우승률 12.0%)과 거의 같은 자리로 돌아온다. 전술은 AI 를 조금 강하게, 흐름은 약팀(멘탈 낮은 연습생)에게 조금 불리하게 작용하는데 둘이 상쇄된다.
- 앱 경제(`--app`) 9/9 (S1 우승 14.5% · 첫 우승 S2~3 50.5%). 장기(`--long --n 120`) **20/20** — 재육성 'best' 로 깨졌던 시즌 9~15 우승률(68~84%)이 61.7~70.8% 로 돌아왔고 골드 잔액도 S4 10,983 → S15 10,822 로 안정된다(league-and-economy D.2).
- 다른 시드: seed 3 · n=400 은 9/9(순위 4.97), seed 7 · n=400 은 시즌 1 순위 **5.03** 으로 목표(≤5.0)를 0.03 넘긴다(기준선 4.91, SE ≈ 0.08). 압박 상수를 낮춰 봐도(`--flow-pressure 0.05` → 5.03, `--flow-relief 0.35` → 5.04) 움직이지 않으므로 흐름 상수가 아니라 구단 전술 + 시드 편차로 본다. 목표를 넓히지 않고 기록해 둔다.
- 한 경기의 체감: 0-3 패배 경기에서 작전타임 9회(세트당 2회 × 양 팀)까지 나온다. 뷰어는 작전타임 배너로 잠깐 멈추고, 결과 화면에 최장 연속·작전타임 횟수 한 줄이 붙는다.

남은 것: 플레이어가 직접 작전타임을 부르고 세트 간 작전을 고르는 것, C# 포팅. 부진·피로 기반 교체는 16절로 들어갔다.

---

## 16. 선수 교체 · 12명 로스터 (v0.4, 웹 엔진)

"엔진 1단계 다음" 으로 예고했던 첫 항목. `config.subs` 블록이고 **기본은 꺼져 있다**(parity·기본 sim 불변). 게임 층이 `SUBS.enabled` 로 켠다.
**RNG 를 소비하지 않는다** — 켜도 난수열은 같고 라인업만 달라진다(판단은 전부 결정적).

| 규칙 | 상수 | 내용 |
|---|---|---|
| 벤치 | `lineup.benchIds` | 리베로가 아닌 벤치. 플레이어 = 로스터에서 라인업 7명을 뺀 전원(연습생 + 남는 졸업생). AI 6구단 = **생성 벤치 4명**(OH·MB·OP·S, `game.js BENCH`), 세기 = min(사다리 OVR − 8, 가장 약한 슬롯 선수 − 2) — 자동 편성은 `isBench` 를 선발 후보에서 빼므로 AI 라인업이 그대로다(캘리브레이션 불변) |
| 부진 교체 | `slumpAttacks 4` · `slumpKillRate 0.22` · `slumpErrors 3` · `slumpRatio 0.92` · `minPoints 6` | 이 세트 공격 4회 이상에 성공률 22% 미만이거나 범실(피블로킹 포함) 3회면 부진. 같은 포지션(OH↔OP 허용) 벤치 중 실효 핵심 레이팅이 선발의 92% 이상인 최고 후보와 바꾼다. 양 팀 합계 6점 전에는 판단하지 않는다 |
| 부진 로짓 | `slumpLogit 0.20` | 부진 중인 공격수는 **킬 로짓 −0.20**(자신감 하락) — 교체되거나 세트가 끝날 때까지. 이것이 있어야 교체가 손해가 아니다: 없으면 "부진" 은 그냥 운이라 더 약한 벤치를 넣는 쪽이 손해였다(측정: 시즌 1 우승률 20.5%, 첫 우승 S2~3 46%) |
| 피로 교체 | `fatigueSet 3` · `fatigueRatio 1.0` | 3세트부터, 벤치 후보의 실효 레이팅(피로·컨디션 반영)이 선발보다 높으면 바꾼다 |
| 한도·복귀 | `perSet 6` | 세트당 6회. 들어온 벤치는 다시 안 빼고, 빠진 선발은 그 세트에 못 돌아온다. 세트가 바뀌면 원래 라인업(`baseStarters`) |
| 핵심 레이팅 | `coreRating` | S 토스 · OH (공격+리시브)/2 · MB (블로킹×2+공격)/3 · OP 공격, 실효 배수 포함 |
| 이벤트 | `EV.Substitution`(15) | playerId = IN, secondary = [OUT], value = 이 세트 몇 번째, reason 1 부진 · 2 피로. 중계·뷰어 배너·결과 화면이 읽는다 |
| 감독 방침 | `state.subsAuto` | 라인업 화면 토글. 끄면 내 팀만 교체하지 않는다(`TeamState.autoSubs`) |

**측정** (`node web/season-check.mjs`, n=200 · seed 1, rev 2단계 시점). `--no-subs --no-bench` 는 v0.3 수치(43.5% · 4.92 · 12.0% · 58.5% · 8923)를 비트 단위로 재현한다. 벤치만 켜도 같다(자동 편성 불변).

| 조합 | S1 승률 | S1 순위 | S1 우승 | 첫 우승 S2~3 | 판정 |
|---|---|---|---|---|---|
| 교체 on, 부진 로짓 없음 | 45.4% | 4.60 | 20.5% | 46.0% | 7/9 |
| **교체 on + 부진 로짓 0.20** | **44.2%** | **4.66** | **13.5%** | **55.5%** | **9/9** |

체감: 경기당 교체 2~3회(AI 는 벤치가 늘 있고, 플레이어는 졸업생이 8명을 넘어야 벤치가 92% 를 맞춘다). 뷰어는 교체 배너로 잠깐 멈추고 결과 화면 흐름 줄에 "교체 · 우리 n회 / 상대 n회" 가 붙는다.

## 17. 매치데이 컨디션 · 경상 (v0.4)

`game.js MATCHDAY`. 엔진의 `playerCondition`(실효 배수)을 게임 층이 채우는 것이라 판정식은 그대로다. 친선 경기는 둘 다 없다.

| 항목 | 상수 | 내용 |
|---|---|---|
| 컨디션 | `levels [0.94, 0.97, 1.00, 1.03, 1.06]` · `baseProb [10, 18, 44, 18, 10]%` · `staminaShift 0.05` | 매치데이마다 선수별로 **결정적**으로(시즌 시드 · 매치데이 · 선수 id 해시) 5단계 중 하나. 스태미나 100 은 나쁨·조금 나쁨이 각 5%p 줄고 좋음·최상이 5%p 는다(0 이면 반대). 결정적이라 **경기 전에 알 수 있다** → 매치데이 화면 "오늘 컨디션", 라인업 화면 배지, 나쁜 선수는 바꿀 수 있다(라인업 슬롯 바꾸기) |
| 경상 | `injuryBase 1.0%` · `injuryLowCond ×2` · `injuryLowStamina ×1.5`(스태미나 < 55) · `injuryDays 1~3` | 경기에 나선 7명(선발 + 리베로, 생성 벤치 제외)이 경기 뒤 결정적으로 롤. 걸리면 `state.injuries[id] = {until, season, days, part}` — 다음 매치데이부터 결장(로스터에서 빠진다). 플레이어는 연습생·벤치가, AI 구단은 생성 벤치가 메운다. 시즌이 바뀌면 낫는다 |
| 빈도 | | 1.2% 기준 시즌당 7팀 합계 8.6건, 내 팀 1.6건(연습생 스태미나가 낮아 조금 많다). 플레이어는 결장자를 연습생으로 메워 AI 보다 더 아프기 때문에 1.0% 로 내렸다 |

## 18. 케미 · 성격 판정 (v0.4)

엔진에 있던 `chemistry` 표(세터|공격수 → 0~100, 기본 50, 토스 품질 로짓 `logitScale 0.5 × (케미−50)/50`)를 게임 층이 채운다. `game.js CHEMISTRY`.

| 항목 | 값 |
|---|---|
| 함께 뛴 경기 | 라인업 7명이 한 경기를 뛰면 세터와의 짝마다 +1(친선 포함) → 경기당 `perGame 2`, 최대 `maxGames 20`(10경기). `state.pairGames` 에 쌓이고 시즌을 넘어 유지된다 |
| 성격 궁합 | 세터 아키타입 × 공격수 아키타입 표 ±6 (`CHEMISTRY.pair`, commentary.js 의 6가지 아키타입). 예: 냉정파 세터 + 노력파 공격수 +5, 승부사 세터 + 냉정파 공격수 −4 |
| AI 구단 | 원소속 슬롯 선수끼리 `clubBase 12`(오래 같이 뛴 사이, 플레이어 6경기 분), 생성 선수(벤치·대체·신인)가 끼면 `newBase 4` |
| 화면 | 라인업 화면 공격수마다 "케미 NN" 배지(≥62 파랑 · <50 빨강), 슬롯 바꾸기 후보에도 |

## 19. AI 구단 운영 (v0.4)

`game.js CLUB_OPS`. 시즌 2부터(`fromSeason 2` — 시즌 1 캘리브레이션 불변) 구단마다 한 방침을 계정 시드·구단·시즌에서 결정적으로 정한다.

| 항목 | 값 |
|---|---|
| 방침 | 공격(오픈 +0.10 · 후위 +0.05) · 수비(리베로 중심 대형, 서브 공격성 −0.05) · 서브(공격성 +0.10) · 균형(속공·지연 +0.05) — `focusTactic 0.10` |
| 집중 육성 | 슬롯 현역 중 (잠재력 − 스탯) 여유가 가장 큰 선수 OVR `keyPlayerOvr 0.75`(그 시즌만, `isKeyPlayer`). 처음 1.5 는 첫 우승 S2~3 을 4.5%p 깎아 반으로 줄였다 |
| 뉴스 | 개막 히스토리 "개막 뉴스 · …" + 매치데이 화면 "상대 소식". 세이브에 넣지 않는다(시드에서 다시 만든다) |

**2단계 합산 측정** (`node web/season-check.mjs --n 500`, seed 1 — n=200 은 첫 우승 S2~3 의 표준오차가 3.5%p 라 500 으로 쟀다).

| 조합 (n=500) | S1 승률 | S1 순위 | S1 우승 | 첫 우승 S2~3 | S3 골드 | 판정 |
|---|---|---|---|---|---|---|
| 2단계 전부 off (`--no-subs --no-bench --no-condition --no-injury --no-chemistry --no-ops`) | 43.0% | 4.90 | 13.8% | 54.0% | 8804 | 9/9 |
| **2단계 전부 on — 채택** | **41.8%** | **4.91** | **12.8%** | **51.2%** | **8657** | **9/9** |
| 케미만 off | 43.5% | 4.79 | 14.6% | 49.8% | 8445 | 7/9 |
| AI 케미 기본 12 → 6 | 41.8% | 4.92 | 13.2% | 52.0% | 8535 | 9/9 |

- 2단계는 플레이어를 **2~3%p** 약하게 만든다(첫 우승 S2~3 54.0 → 51.2, 골드 8804 → 8657). 원인은 대칭이 아닌 부분 — 시즌 1 의 결장자를 플레이어는 연습생(OVR 44)으로, AI 는 벤치(사다리 −8)로 메우고, 컨디션 분산은 강한 쪽에 조금 불리하다. 목표 안이라 그대로 두고 기록한다.
- 케미는 오히려 플레이어 편이다(끄면 49.8%): 함께 뛴 경기가 쌓이면 +20 으로 AI 의 +12 를 넘는다. AI 케미 기본을 6 으로 낮춰도 표본 오차 안이라 12 를 유지.
- 앱 경제 `--app`(n=200) 9/9 — 44.8% · 4.67 · 15.5% · 50.0% · 8811. 시스템별 A/B(n=200, 잡음 큼): 운영 off 55.0% · 경상 off 52.5% · 컨디션 off 48.0%.
- 다른 시드: seed 3 · n=400 은 41.2% · 4.96 · 8.8% · 55.5% · **골드 8453**(목표 8500, 도입 전 8613 → −160). 2단계는 시즌 3 골드를 150 안팎 깎는다 — 교체가 내 주전의 득점을 나눠 시즌 타이틀(MVP 1,000 · 부문 500골드)이 조금 덜 온다는 것이 유력한 원인. 목표를 넓히지 않고 기록해 둔다(레버 후보: 자동 교체 off 는 플레이어의 선택이다).
- 장기 `--long --n 120`: 시즌 4~15 목표 **20/20**(우승률 46~63% 고원, 골드 S4 11,170 → S15 11,065). 같은 실행의 시즌 1~3 "첫 우승 S2~3" 45.8% 는 n=120 표본 잡음(n=500 은 51.2%).
